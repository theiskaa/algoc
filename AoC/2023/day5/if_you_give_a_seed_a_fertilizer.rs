 ...
 * forgot to check why we stopped getting more sand!
 * There's a ferry leaving soon that is headed over in that direction - it's
 ...
 * kind of fertilizer, and so on. Every type of seed, soil, fertilizer and
 * so on is identified with a number, but numbers are reused by each category - that is,
 ...
 * (the destination). This lets the gardener and his team know which soil to use
 * with which seeds, which water to use with which fertilizer, and so on.
 *
 * Rather than list every source number and its corresponding destination
 * number one by one, the maps describe entire ranges of numbers that can be converted.
 ...
 * length, but it starts at 50, so its two values are 50 and 51.
 ...
 * Using these maps, find the lowest location number that corresponds to any
