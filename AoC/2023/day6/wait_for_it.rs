 ...
 * you will get a fixed amount of time during which your boat has to travel as
 ...
 * lists the time allowed for each race and also the best distance ever recorded in
 * that race. To guarantee you win the grand prize, you need to make sure
 ...
 * the button counts against the total race time.
 ...
 *    Please make sure you let go of the button so the boat gets to move. 1 millimeters.
 *
 * Since the current record for this race is 9 millimeters, there are actually
 * 4 different ways you could win: you could hold the button for 2, 3, 4, or 5 milliseconds at the start of the race.
 *
...
fn main() {
    ...
    println!("AoC:2023 • Day 6 • Wait For It\nResult: {}", result)
}
...
// Parses the [input.txt] file to TimeDistance
fn parse_input(file: File) -> TimeDistance {
    let reader = BufReader::new(file);
    let lines: Vec<String> = reader.lines().map(|l| l.unwrap()).collect();
    let time: Vec<i32> = lines[0].split_whitespace().skip(1).map(|s| s.parse().unwrap()).collect();
    let distance: Vec<i32> = lines[1].split_whitespace().skip(1).map(|s| s.parse().unwrap()).collect();
    return TimeDistance { time, distance };
}

fn wait_for_it(input: TimeDistance) -> i32 {
    let mut result: i32 = 1;
    for (i, n) in input.time.iter().enumerate() {
        let mut ways = 0;
        for time in 0..*n {
            let distance = time * (n - time);
            if distance > input.distance[i] {
                ways += 1;
            }
        }
        result *= ways;
