 ...
 * after all! I bet the gardener would know.
 ...
 * of the numbers you have appear in the list of winning numbers.
...
fn main() {
    ...
    println!("AoC:2023 • Day 4 • Scratchcards\nResult: {}", result)
}
...
// Parses the [input.txt] file to the Vector of Scratchcard(s)
fn parse_input(file: File) -> Vec<Scratchcard> {
    let reader = BufReader::new(file);
    let mut res: Vec<Scratchcard> = Vec::new();
    for line in reader.lines() {
        let current = line.unwrap();
        let data: String = current.split_at(current.find(':').unwrap() + 1).1.trim().to_string();
        let (winning, all) = data.split_once(" | ").unwrap();
        let winning: Vec<i32> = winning.trim().split_whitespace().map(|n| n.parse::<i32>().unwrap()).collect();
        let all: Vec<i32> = all.trim().split_whitespace().map(|n| n.parse::<i32>().unwrap()).collect();
        res.push(Scratchcard { winning, all } );
    }

    return res;
}

fn scratchcards(input: Vec<Scratchcard>) -> i32 {
    let mut total_points: i32 = 0;
    for scratchcard in input {
        let mut card_points: i32 = 0;
        let mut match_count: i32 = 0;

        for &num in &scratchcard.all {
            if scratchcard.winning.contains(&num) {
                match_count += 1;
                if match_count == 1 {
                    card_points = 1;
                } else {
