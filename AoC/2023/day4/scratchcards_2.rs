// 2023 - Day 4 - Scratchcards | Part Two
...
fn main() {
    ...
    println!("AoC:2023 • Day 4 • Scratchcards | Part Two\nResult: {}", result)
}
...
fn scratchcards(input: &Vec<Scratchcard>) -> u32 {
    let mut counts = vec![1; input.len()];

    for (index, card) in input.iter().enumerate() {
        let winners = card.winning.iter().filter(|s| card.all.contains(s)).count();
        let cards = counts[index];
