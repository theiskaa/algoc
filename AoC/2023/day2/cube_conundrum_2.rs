// 2023 - Day 2 - Cube Conundrum | Part Two
...
fn main() {
    ...
    println!("AoC:2023 • Day 2 • Cube Conundrum | Part Two\nResult: {}", result)
}
...
fn parse_input(file: File) -> Vec<String> {
    ...
    for line in reader.lines() {
        ...
        let game_data: Vec<&str> = current.split(':').collect();
        if game_data.len() > 1 {
            res.push(game_data[1].trim().to_string());
        }
    }
    return res;
}

fn cube_conundrum(input: Vec<String>) -> i32 {
    let mut result: i32 = 0;

    for line in input {
        let mut records: HashMap<&str, i32> = HashMap::new();
        ...
        for set in sets {
            ...
            for cube in cubes {
                ...
                records.entry(color).and_modify(|v| *v = (*v).max(count)).or_insert(count);
            }
        }

        let red = *records.get("red").unwrap_or(&0);
        let green = *records.get("green").unwrap_or(&0);
        let blue = *records.get("blue").unwrap_or(&0);
